<?php 
    ...
    if($mode == "connexion"){
            ...
            //  Requete pour tester si le pseudo est déja présent dans la BDD
            $resultat = $pdo->query("SELECT * FROM membre WHERE pseudo = '$pseudo' ");
            $membre = $resultat->fetch(PDO::FETCH_ASSOC);
            if($resultat->rowCount() == 0) {  // s'il n'y a pas de ligne alors est libre inexistant en BDD
                $time = time();
                ...
                $tab['resultat'] = "membre registred !";

            } elseif ($resultat->rowCount() > 0 && $_SERVER['REMOTE_ADDR'] == $membre['ip']) {
                    // si le pseudo existe et que l'adresse ip est la même que celle enregistrée, c'est donc le même user
                    ...
                    // le pseudo est deja pris et l'adresse ip ne correspond pas à ce pseudo 
                    $tab['resultat'] = "Pseudo indisponible, veuillez recommencer !";
                    $erreur = true; // nous sommes dans un d'erreur, nous changeons la valeur de cette variable pour la tester après. 
                    $tab['pseudo'] = "indisponible"; // évite la redirection depuis index.php 
            }

            if(!$erreur){  // if($erreur == false) // s'il n'y a pas d'erreur durant les traitement précédant. 
