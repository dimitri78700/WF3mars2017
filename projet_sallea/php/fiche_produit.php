<?php
    ...
    if (isset($_GET['id_salle'])){ // Si existe l'indicee id-produit dans l'URL
        ...
        $contenu .= '<div class"col-md-4">';
            ...
            $contenu .= '<p><a href="sallea.php?categories='. $produit['categories'] .'">Retour vers votre sélection</a></p>';
        $contenu .= '</div>';
    }
    else{
    ...
    $suggestion = executeRequete("SELECT id_salle, photo, titre FROM produit WHERE categories = '$produit[categories]' AND id_salle != '$_GET[id_salle]' ORDER BY RAND() LIMIT 2");       
